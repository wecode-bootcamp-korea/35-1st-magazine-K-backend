...
class CartView(View):
    ...
    def get(self, request):
        user = request.user
        cart_products = OrderItem.objects.filter(
            order__user=user, order__order_status=OrderStatusEnum.CART.value
        )
            ...
            with transaction.atomic():
                if not cart_order.exists():
                    new_order = Order.objects.create(
                        user=user, order_status_id=OrderStatusEnum.CART.value
                    )
    ...
    def patch(self, request, product_id):
        try:
            ...
            selected_product = Product.objects.get(id=product_id)
            cart_product = OrderItem.objects.filter(
                order__user=user,
                product_id=selected_product,
                order__order_status=OrderStatusEnum.CART.value,
            )
    ...
    def delete(self, request, product_id):
        try:
            user = request.user
            cart_product = OrderItem.objects.get(
                order__user=user,
                product_id=product_id,
                order__order_status_id=OrderStatusEnum.CART.value,
            )

            cart_product.delete()

            if not Order.objects.get(
                user=user, order_status_id=OrderStatusEnum.CART.value
            ).orderitem_set.exists():
        ...
        try:
            ...
            with transaction.atomic():
                ...
                User.objects.filter(id=user.id).update(
                    point=User.objects.get(id=user.id).point - price_total
                )
